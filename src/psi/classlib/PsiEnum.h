using namespace psi;
#define psi_enum(class_name, ...) \
  class class_name : public Enum { \
    public: \
            enum __psi_##class_name { \
              __VA_ARGS__ \
            }; \
    class_name() : Enum(){ parse_name_value (#__VA_ARGS__); } \
    class_name(__psi_##class_name e) : class_name() { set(e); } \
    class_name(const Scope& p) : Enum(this) { parse_name_value (#__VA_ARGS__); } \
    __psi_##class_name get() const \
    { \
      return Enum::get<__psi_##class_name>(); \
    } \
    void set(__psi_##class_name e) \
    { \
      Enum::set<__psi_##class_name>(e); \
    } \
    class_name& operator=(const __psi_##class_name e) \
    { \
      Enum::operator=(e); \
      return *this; \
    } \
...
#define psi_enum_ext(class_name, base_class, ...) \
