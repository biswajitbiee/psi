...
BaseItem* Elaborator::getTypeDecl(psi_api::insthandle_t handle) {
    IBaseItem* bi = m_model->getTypeDecl(handle);
    return m_ibaseitem2type_map[bi]->master();
}
...
IGraphStmt *Elaborator::elaborate_graph(GraphImp *g) {
	...
	ExprCoreList& stmts = static_cast<ExprCoreList&>(*stmt_tree.imp().ptr());
	...
	if (stmts.getExprList().size() > 1 || constraints.size() > 0) {
		...
		for (it=stmts.getExprList().begin();
				it!=stmts.getExprList().end(); it++) {
    ...
    if(constraints.size() > 0)
    {
      ...
      {
        constraint_stmt->add(elaborate_constraint(static_cast<ConstraintImp*>(c)));
      }
...
IGraphStmt *Elaborator::elaborate_graph_stmt(ExprCore *stmt) {
  ...
  IGraphStmt::GraphStmtType stmt_type = IGraphStmt::GraphStmt_Block;
      ...
      case ExprImp::Tree: 
        {
          ...
          if(op == ExprImp::GraphSelect) {
            stmt_type = IGraphStmt::GraphStmt_Select; 
          } else if(op == ExprImp::GraphParallel) {
            stmt_type = IGraphStmt::GraphStmt_Parallel;
          } else if (op == ExprImp::GraphSchedule) {
            stmt_type = IGraphStmt::GraphStmt_Schedule;
          }
	...
	case ExprImp::TypeRef: {
		...
		if (ref) {
			FieldItemImp *fi = static_cast<FieldItemImp*>(stmt->getTypePtr());
      		IConstraint* c = nullptr;
      		if(fi->get_constraint())
      		{
        		c = elaborate_constraint(fi->get_constraint());
      		}
